...
Audio::Audio(std::string folder) {
  ...
  if (!(inGameSong = al_load_sample(temp.c_str())))
    throw "Ingamesong not found! \nWe suggest to check if filename is correct "
...
// Play jump sound
void Audio::jump(float Volume, float Pan) {
  al_play_sample(jumpSound, Volume / 100, Pan, 1.0, ALLEGRO_PLAYMODE_ONCE,
                 NULL);
}

// Play end sound
void Audio::death(float Volume, float Pan) {
  al_play_sample(endSound, Volume / 100, Pan, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
}

// Start menu Song loop, stop it and return value to check if menu song is
// playing
void Audio::loopMenu(float Volume, float Pan) {
  al_play_sample(menuSong, Volume / 100, Pan, 1.0, ALLEGRO_PLAYMODE_LOOP,
                 &MenuMusic);
  this->menuPlaying = true;
}
void Audio::stopLoopMenu() {
  al_stop_sample(&MenuMusic);
  this->menuPlaying = false;
}
bool Audio::isMenuPlaying() { return this->menuPlaying; }

// Start inGame Song loop, stop it and return value to check if inGame song is
// playing
void Audio::loopInGame(float Volume, float Pan) {
  al_play_sample(inGameSong, Volume / 100, Pan, 1.0, ALLEGRO_PLAYMODE_LOOP,
                 &InGameMusic);
  this->InGamePlaying = true;
}
void Audio::stopLoopInGame() {
  al_stop_sample(&InGameMusic);
  this->InGamePlaying = false;
}
