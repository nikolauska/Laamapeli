...
void Ground::create(int &lastX, int &lastY, int speed) {
  ...
  // this next confusing function will make sure player will not have impossible
  // jump
  ...
  this->x =
      lastX + int((std::rand() % 100 + 1)) - speed; // Set random X postion
...
// Move ground to left
void Ground::move(int speed) { this->x -= speed; }
