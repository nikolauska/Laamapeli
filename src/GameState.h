...
public:
	...
	void startGame();
	void recycleGround(Ground& ground, int speed) { ground.create(lastX_, lastY_, speed); }
	void drawGrounds();
	void moveGrounds();
	...
	void setResolutionPosition(std::size_t position) { resolutionPosition_ = position; }
	int resolutionWidth() const { return resolutionWidths_[resolutionPosition_]; }
	int resolutionHeight() const { return resolutionHeights_[resolutionPosition_]; }
...
private:
	...
	bool isFpsTimerEvent(const ALLEGRO_EVENT& event) const { return event.timer.source == fpsTimer_.get(); }
	bool isScoreTimerEvent(const ALLEGRO_EVENT& event) const { return event.timer.source == scoreTimer_.get(); }
	...
	int lastX_ = 0;
	int lastY_ = 0;
	std::array<int, 10> resolutionWidths_{640, 800, 1024, 1280, 1280, 1366, 1600, 1600, 1680, 1920};
	std::array<int, 10> resolutionHeights_{480, 600, 768, 720, 960, 768, 900, 1200, 1050, 1080};
