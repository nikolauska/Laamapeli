...
void GameState::findResolution() {
  ...
  for (std::size_t i = 0; i != resolutionWidths_.size(); i++)
    if (resolutionWidths_[i] == settings.width &&
        resolutionHeights_[i] == settings.height) {
...
void GameState::startGame() {
  ...
  grounds_.back().start(settings.width, settings.height, draw_->playerHeight(),
                        lastX_, lastY_);
...
void GameState::moveGrounds() {
  ...
  for (auto &ground : grounds_) {
    ...
    if (ground.getX() - 300 <= (tempX - tempW * 1.5) &&
        ground.getX() + 300 - tempW / 2 >= tempX)
      if (player_.isFalling() && previousY + tempH <= ground.getY() &&
          tempY + tempH >= ground.getY())
...
void loadingScreen(const std::string &text, const Settings &settings,
                   const std::shared_ptr<ALLEGRO_BITMAP> &loading,
                   const std::shared_ptr<ALLEGRO_FONT> &fontLoading) {
  al_draw_scaled_bitmap(loading.get(), 0, 0, al_get_bitmap_width(loading.get()),
                        al_get_bitmap_height(loading.get()), 0, 0,
                        settings.width, settings.height,
                        0); // draw loading image
  al_draw_text(fontLoading.get(), al_map_rgb(255, 0, 0), settings.width / 2,
               settings.height / 8 * 7, ALLEGRO_ALIGN_CENTER,
               text.c_str()); // draw text
...
int GameState::initialize() {
  ...
  if (!loadingBitmap) {
    al_show_native_message_box(
        display_.get(), "ERROR", "IMAGES",
        "Backround for loadingscreen not found! \nWe suggest to check if "
  ...
  auto loading =
      std::shared_ptr<ALLEGRO_BITMAP>(loadingBitmap, al_destroy_bitmap);
  ...
  if (!loadingFont) {
    al_show_native_message_box(
        display_.get(), "ERROR", "FONT",
        "Font not found! \nWe suggest to check if filename is correct in "
        "'Font' folder. \nFilename for this file is 'font.ttf'",
        NULL, 0);
    display_.reset();
    return -1;
  }
  auto fontLoading =
      std::shared_ptr<ALLEGRO_FONT>(loadingFont, al_destroy_font);
  ...
  try {
    ...
    al_show_native_message_box(display_.get(), "ERROR", "IMAGES", message, NULL,
                               0);
    display_.reset();
    player_ = Player{};
    return -1;
  ...
  try {
    ...
    al_show_native_message_box(display_.get(), "ERROR", "AUDIO", message, NULL,
                               0);
    display_.reset();
    player_ = Player{};
    draw_.reset();
  ...
  eventQueue_ =
      std::shared_ptr<ALLEGRO_EVENT_QUEUE>(eventQueue, al_destroy_event_queue);
  ...
  al_register_event_source(eventQueue_.get(),
                           al_get_display_event_source(display_.get()));
  al_register_event_source(eventQueue_.get(),
                           al_get_timer_event_source(fpsTimer_.get()));
  al_register_event_source(eventQueue_.get(),
                           al_get_timer_event_source(scoreTimer_.get()));
