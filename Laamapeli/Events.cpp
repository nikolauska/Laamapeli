							...
							case(3):{ // Graphics menu
								switch (state.menuSelection()){
									case(1):{ // top
										...
										settings.saveDisplay(state.resolutionWidth(), state.resolutionHeight(), settings.pendingFPS);
										state.destroy();
										state.initialize();
										break;
									}
							...
							switch(state.menuSelection()){
								case(1):{ // top
									if(state.resolutionPosition() != 0) // if not 0
										state.setResolutionPosition(state.resolutionPosition() - 1); // then minus 1
									break;
								}
								case(2):{ // middle
				...
				if(state.gamePosition() == 1) {
					switch(state.menuText()){
						case(3):{ // Graphics
							switch(state.menuSelection()){
								case(1):{ // top 
									if(state.resolutionPosition() != state.resolutionCount() - 1)	// if not end of list
										state.setResolutionPosition(state.resolutionPosition() + 1);				// then add 1
									break;
								}
								case(2):{ // middle
					...
					switch(state.menuText()){
						case(3):{ // graphics menu
							if(ev.mouse.dz > 0)					// mouse scroll forward
								if(state.resolutionPosition() != state.resolutionCount() - 1)	// if not last of list
									state.setResolutionPosition(state.resolutionPosition() + 1);				// then add 1
								else
									state.setResolutionPosition(0);					// add to start
							else if(ev.mouse.dz < 0)			// mouse scroll backward
								if(state.resolutionPosition() != 0)					// if not 0
									state.setResolutionPosition(state.resolutionPosition() - 1);				// minus 1
								else
									state.setResolutionPosition(state.resolutionCount() - 1);	// set to end
							break;
						}
						case(4):{ // audio menu 
							if(ev.mouse.dz > 0)			// mouse scroll forward 		
...
void drawEvent(GameState& state){
	...
	switch(state.gamePosition()){
		case(1):{ // menu
			...
			// draw menu screen
			state.draw().menu(state.menuText(), state.menuSelection(), state.resolutionWidth(), state.resolutionHeight(), settings.pendingFPS, settings.pendingVolume, settings.pendingPan);
			break;
		}
		case(2):{ // start
		...
		case(3):{ // ingame
			...
			if (state.player().getY() + state.draw().playerHeight() >= settings.height) {
